...
public class HardwareUtil {
	 ...
	 * 
	 * @return					First queue family index that meets all requirements or <b>-1</b> if no such queue exist.
	 * @throws VulkanException	when surface support query fails.
	 */
	public static int getNextPresentQueueFamilyIndex(int first, int requiredFlags, long surface, VkPhysicalDevice physicalDevice, VkQueueFamilyProperties.Buffer queueFamilyProperties) throws VulkanException {
		IntBuffer supportPresent = memAllocInt(1);
		
		for(int i = first; i < queueFamilyProperties.remaining(); i++) {
			supportPresent.position(0);
			
			int err = vkGetPhysicalDeviceSurfaceSupportKHR(physicalDevice, i, surface, supportPresent);
	 ...
	 * @return		A buffer of queue family properties. 
	 * 				This buffer <b>should</b> be freed by the implementation.
	 */
	public static VkQueueFamilyProperties.Buffer newQueueFamilyProperties(VkPhysicalDevice physicalDevice) {
		...
		vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, queue_family_count_buffer, queueFamilyProperties);
	 ...
	 * 
	 * @param physicalDevice	<b>Must</b> be a valid pointer to VkPhysicalDevice.
	 * @return		A VkPhysicalDeviceMemoryProperties object.
	 * 				This object <b>should</b> be freed by the implementation.
	 */
	public static VkPhysicalDeviceMemoryProperties newPhysicalDeviceMemoryProperties(VkPhysicalDevice physicalDevice) {

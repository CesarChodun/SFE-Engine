...
/**
 * Class for transferring information from a JSON file to a class.
 *
...
public class JSONParser {
     ...
     * @return If the method exists it is returned, otherwise null is returned.
     */
    ...
    /**
     * Compares two classes, and checks whether they are interchangeable.
     *
    ...
    private static boolean compareClass(Class<?> c1, Class<?> c2) {
        ...
        if (c1.equals(Integer.class)) return int.class.equals(c2);
        if (c1.equals(Boolean.class)) return boolean.class.equals(c2);
        if (c1.equals(Float.class)) return float.class.equals(c2);
        if (c1.equals(Long.class)) return long.class.equals(c2);

        if (c2.equals(Integer.class)) return int.class.equals(c1);
        if (c2.equals(Boolean.class)) return boolean.class.equals(c1);
        if (c2.equals(Float.class)) return float.class.equals(c1);
        if (c2.equals(Long.class)) return long.class.equals(c1);
     ...
     * @param name Name of the method.
     * @param params Parameter types.
     * @return If a method with interchangeable parameters exists and has the same name it is
            ...
            if (m.getName().equals(name)) {
                ...
                if (original.length == params.length) {
                    for (int i = 0; i < params.length; i++)
                        if (!compareClass(original[i], params[i])) return null;
                    return m;
                }
            }
    ...
    public static void populateData(Object obj, JSONObject json) {
        if (obj == null) {
            if (Application.DEBUG)
                logger.log(
                        Level.WARNING,
                        "Failed to load data from JSON file! "
            ...
            if (!s.equals(CLASS_IDENTIFIER)) {
                ...
                if (JSONObject.class.isInstance(toAdd)) {
                    String getMethodName =
                            GET_PREF + s.substring(0, 1).toUpperCase() + s.substring(1);
                    Method getMethod = null;
                    Object sub = null;

                    try {
                        getMethod =
                                searchForMethod(obj.getClass(), getMethodName, (Class<?>[]) null);

                        if (getMethod != null) sub = getMethod.invoke(obj, (Object[]) null);

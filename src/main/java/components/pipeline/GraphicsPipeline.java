...
import main.java.core.resources.ConfigAsset;
import main.java.core.resources.ConfigFile;
import main.java.core.resources.ResourceUtil;
...
public class GraphicsPipeline implements Pipeline {
    ...
    private static final String VERTEX_INPUT_STATE_KEY = "pVertexInputState",
    ...
    private static final String VIB_BINDING_KEY = "binding",
            VIB_STRIDE_KEY = "stride",
            VIB_INPUT_RATE_KEY = "inputRate";

    private static final Integer DEFAULT_VIB_BINDING = 0, DEFAULT_VIB_STRIDE = 2 * 4;

    private static final String DEFAULT_VIB_INPUT_RATE = "VK_VERTEX_INPUT_RATE_VERTEX";

    // Vertex Input Attribute Description

    private static final String VIA_BINDING_KEY = "binding",
    ...
    private static final Integer DEFAULT_VIA_BINDING = 0,
            DEFAULT_VIA_LOACATION = 0,
            DEFAULT_VIA_OFFSET = 0;

    // Input Assembly

    private static final String IA_TOPOLOGY_KEY = "topology",
    ...
    private static final String TS_PATCH_CONTROL_POINTS_KEY = "patchControllPoints",
    ...
    private static final String RS_POLYGON_MODE_KEY = "polygonMode",
    ...
    private static final Boolean DEFAULT_CB_BLEND_ENABLE = false;

    private static final Integer DEFAULT_CB_COLOR_WRITE_MASK = 0xF;

    private static final String DEFAULT_CB_SRC_COLOR_BLEND_FACTOR = "VK_BLEND_FACTOR_ZERO",
    ...
    private static final String MS_RASTERIZATION_SAMPLES_KEY = "rasterizationSamples",
    ...
    private static final String DEFAULT_MS_RASTERIZATION_SAMPLES = "VK_SAMPLE_COUNT_1_BIT";

    private static final Boolean DEFAULT_MS_SAMPLE_SHADING_ENABLE = false,
            DEFAULT_MS_ALPHA_TO_COVERAGE_ENABLE = false,
            DEFAULT_MS_ALPHA_TO_ONE_ENABLE = false;

    private static final Float DEFAULT_MS_MIN_SAMPLE_SHADING = 0.0f;

    // Viewport state

    private static final String VP_VIEWPORT_COUNT_KEY = "viewportCount",
            VP_SCISSORS_COUNT_KEY = "scissorsCount";
    private static final Integer DEFAULT_VP_VIEWPORT_COUNT = 1, DEFAULT_VP_SCISSOR_COUNT = 1;

    // Depth Stencil State

    private static final String DS_DEPTH_TEST_ENABLE_KEY = "depthTestEnable",
    ...
    private static final String DEFAULT_DS_DEPTH_COMPARE_OP = "VK_COMPARE_OP_ALWAYS",
    ...
    private static final Boolean DEFAULT_DS_DEPTH_TEST_ENABLE = false,
            DEFAULT_DS_DEPTH_WRITE_ENABLE = false,
            DEFAULT_DS_DEPTH_BOUND_ENABLE = false,
            DEFAULT_DS_STENCIL_TEST_ENABLE = false;
    private static final Float DEFAULT_DS_MIN_DEPTH_BOUNDS = 0.0f,
    ...
    private static final String DEFAULT_STAGE = "VK_SHADER_STAGE_VERTEX_BIT",
            DEFAULT_FILE = "storage/res/shaders/triangle.vert.spv",
            DEFAULT_MAIN = "main";

    // Dynamic states

    private static final String DYNAMIC_STATES_KEY = "dynamicStates";

    private static final List<String> DEFAULT_DYNAMIC_STATES = new ArrayList<String>();
    ...
    /**
     * Creates a graphics pipeline with data taken from a default pipeline info file from the asset.
     *
     * @param asset
     * @param configFile
     * @param device
    ...
    public GraphicsPipeline(
            Asset asset, String configFile, VkDevice device, long renderPass, long pipelineLayout)
            throws IOException, AssertionError, VulkanException {
        this.asset = asset;
    ...
    private void loadPipeline(long renderPass) throws IOException, AssertionError, VulkanException {
        ...
        int err =
                vkCreateGraphicsPipelines(
                        device, VK_NULL_HANDLE, pipelineCreateInfo, null, pPipeline);
        ...
        for (int i = 0; i < bindings.size(); i++) {
            ...
            bindingDescription
                    ...
                    .binding(
                            sCfg.getInteger(
                                    VIB_BINDING_KEY,
                                    DEFAULT_VIB_BINDING)) // <- we bind our vertex buffer to point 0
                    .stride(sCfg.getInteger(VIB_STRIDE_KEY, DEFAULT_VIB_STRIDE)) //
                    .inputRate(
                            sCfg.getStaticIntFromClass(
                                    VK10.class, VIB_INPUT_RATE_KEY, DEFAULT_VIB_INPUT_RATE));
        }
        ...
        for (int i = 0; i < attributes.size(); i++) {
            ...
            attributeDescription
                    ...
                    .location(
                            sCfg.getInteger(
                                    VIA_LOCATION_KEY,
                                    DEFAULT_VIA_LOACATION)) // Location 0 : Position in shader
                    .format(
                            sCfg.getStaticIntFromClass(
                                    VK10.class, VIA_FORMAT_KEY, DEFAULT_VIA_FORMAT))
                ...
                VkPipelineInputAssemblyStateCreateInfo.calloc()
                        ...
                        .topology(
                                cfg.getStaticIntFromClass(
                                        VK10.class, IA_TOPOLOGY_KEY, DEFAULT_IA_TOPOLOGY))
                        .primitiveRestartEnable(
                                cfg.getBoolean(
                                        IA_PRIMITIVE_RESTART_ENABLE,
                                        DEFAULT_PRIMITIVE_RESTART_ENABLE));
    }
    ...
    private void loadTessellationState(ConfigAsset cfg) {
        ...
        if (cfg.getBoolean(OMMIT_TESSELLATION_STATE_KEY, DEFAULT_OMMIT_TESSELLATION_STATE)) {
            ...
            tessellationInfo =
                    VkPipelineTessellationStateCreateInfo.calloc()
                            .sType(VK_STRUCTURE_TYPE_PIPELINE_TESSELLATION_STATE_CREATE_INFO)
                            .patchControlPoints(
                                    cfg.getInteger(
                                            TS_PATCH_CONTROL_POINTS_KEY,
                                            DEFAULT_TS_PATCH_CONTROL_POINTS));
        }
    }
    ...
    private void loadRasterizationState(ConfigAsset cfg) {
        ...
        rasterizationStateInfo =
                VkPipelineRasterizationStateCreateInfo.calloc()
                        ...
                        .polygonMode(
                                cfg.getStaticIntFromClass(
                                        VK10.class, RS_POLYGON_MODE_KEY, DEFAULT_RS_POLYGON_MODE))
                        .cullMode(
                                cfg.getStaticIntFromClass(
                                        VK10.class, RS_CULL_MODE_KEY, DEFAULT_RS_CULL_MODE))
                        .frontFace(
                                cfg.getStaticIntFromClass(
                                        VK10.class, RS_FRONT_FACE_KEY, DEFAULT_RS_FRONT_FACE))
                        .lineWidth(cfg.getFloat(RS_LINE_WIDTH_KEY, DEFAULT_RS_LINE_WIDTH))
                        .depthClampEnable(
                                cfg.getBoolean(
                                        RS_DEPTH_CLAMP_ENABLE_KEY, DEFAULT_RS_DEPTH_CLAMP_ENABLE))
                        .rasterizerDiscardEnable(
                                cfg.getBoolean(
                                        RS_RASTERIZER_DISCARD_ENABLE_KEY,
                                        DEFAULT_RS_RASTERIZER_DISCARD_ENABLE))
                        .depthBiasEnable(
                                cfg.getBoolean(
                                        RS_DEPTH_BIAS_ENABLE_KEY, DEFAULT_RS_DEPTH_BIAS_ENABLE))
                        .depthBiasConstantFactor(
                                cfg.getFloat(
                                        RS_DEPTH_BIAS_CONSTANT_FACTOR_KEY,
                                        DEFAULT_RS_DEPTH_BIAS_CONSTANT_FACTOR))
                        .depthBiasClamp(
                                cfg.getFloat(RS_DEPTH_BIAS_CLAMP_KEY, DEFAULT_RS_DEPTH_BIAS_CLAMP))
                        .depthBiasSlopeFactor(
                                cfg.getFloat(
                                        RS_DEPTH_BIAS_SLOPE_FACTOR_KEY,
                                        DEFAULT_RS_DEPTH_BIAS_SLOPE_FACTOR));
    }
        ...
        for (int i = 0; i < attachmentStates.size(); i++) {
            colorWriteMask
                    ...
                    .srcColorBlendFactor(
                            cfg.getStaticIntFromClass(
                                    VK10.class,
                                    CB_SRC_COLOR_BLEND_FACTOR_KEY,
                                    DEFAULT_CB_SRC_COLOR_BLEND_FACTOR))
                    .dstColorBlendFactor(
                            cfg.getStaticIntFromClass(
                                    VK10.class,
                                    CB_DST_COLOR_BLEND_FACTOR_KEY,
                                    DEFAULT_CB_DST_COLOR_BLEND_FACTOR))
                    .colorBlendOp(
                            cfg.getStaticIntFromClass(
                                    VK10.class, CB_COLOR_BLEND_OP_KEY, DEFAULT_CB_COLOR_BLEND_OP))
                    .srcAlphaBlendFactor(
                            cfg.getStaticIntFromClass(
                                    VK10.class,
                                    CB_SRC_ALPHA_BLEND_FACTOR_KEY,
                                    DEFAULT_CB_SRC_ALPHA_BLEND_FACTOR))
                    .dstAlphaBlendFactor(
                            cfg.getStaticIntFromClass(
                                    VK10.class,
                                    CB_DST_ALPHA_BLEND_FACTOR_KEY,
                                    DEFAULT_CB_DST_ALPHA_BLEND_FACTOR))
                    .alphaBlendOp(
                            cfg.getStaticIntFromClass(
                                    VK10.class, CB_ALPHA_BLEND_OP_KEY, DEFAULT_CB_ALPHA_BLEND_OP))
                    .colorWriteMask(
                            cfg.getInteger(CB_COLOR_WRITE_MASK, DEFAULT_CB_COLOR_WRITE_MASK));
        }
        ...
        multisampleState =
                VkPipelineMultisampleStateCreateInfo.calloc()
                        ...
                        .rasterizationSamples(
                                cfg.getStaticIntFromClass(
                                        VK10.class,
                                        MS_RASTERIZATION_SAMPLES_KEY,
                                        DEFAULT_MS_RASTERIZATION_SAMPLES))
                        .sampleShadingEnable(
                                cfg.getBoolean(
                                        MS_SAMPLE_SHADING_ENABLE_KEY,
                                        DEFAULT_MS_SAMPLE_SHADING_ENABLE))
                        .minSampleShading(
                                cfg.getFloat(
                                        MS_MIN_SAMPLE_SHADING_KEY, DEFAULT_MS_MIN_SAMPLE_SHADING))
                        .alphaToCoverageEnable(
                                cfg.getBoolean(
                                        MS_ALPHA_TO_COVERAGE_ENABLE_KEY,
                                        DEFAULT_MS_ALPHA_TO_COVERAGE_ENABLE))
                        .alphaToOneEnable(
                                cfg.getBoolean(
                                        MS_ALPHA_TO_ONE_ENABLE, DEFAULT_MS_ALPHA_TO_ONE_ENABLE));
    }
    ...
    private void loadViewportState(ConfigAsset cfg) {
        viewportStateCreateInfo =
                VkPipelineViewportStateCreateInfo.calloc()
                        ...
                        .viewportCount(
                                cfg.getInteger(VP_VIEWPORT_COUNT_KEY, DEFAULT_VP_VIEWPORT_COUNT))
                        .scissorCount(
                                cfg.getInteger(VP_SCISSORS_COUNT_KEY, DEFAULT_VP_SCISSOR_COUNT));
    }
    ...
    private void loadDepthStencilState(ConfigAsset cfg) {
        depthStencilState =
                VkPipelineDepthStencilStateCreateInfo.calloc()
                        ...
                        .depthTestEnable(
                                cfg.getBoolean(
                                        DS_DEPTH_TEST_ENABLE_KEY, DEFAULT_DS_DEPTH_TEST_ENABLE))
                        .depthWriteEnable(
                                cfg.getBoolean(
                                        DS_DEPTH_WRITE_ENABLE_KEY, DEFAULT_DS_DEPTH_WRITE_ENABLE))
                        .depthCompareOp(
                                cfg.getStaticIntFromClass(
                                        VK10.class,
                                        DS_DEPTH_COMPARE_OP_KEY,
                                        DEFAULT_DS_DEPTH_COMPARE_OP))
                        .depthBoundsTestEnable(
                                cfg.getBoolean(
                                        DS_DEPTH_BOUNDS_TEST_ENABLE_KEY,
                                        DEFAULT_DS_DEPTH_BOUND_ENABLE))
                        .stencilTestEnable(
                                cfg.getBoolean(
                                        DS_STENCIL_TEST_ENABLE_KEY, DEFAULT_DS_STENCIL_TEST_ENABLE))
                        .minDepthBounds(
                                cfg.getFloat(DS_MIN_DEPTH_BOUNDS_KEY, DEFAULT_DS_MIN_DEPTH_BOUNDS))
                        .maxDepthBounds(
                                cfg.getFloat(DS_MAX_DEPTH_BOUNDS_KEY, DEFAULT_DS_MAX_DEPTH_BOUNDS));

        ConfigAsset frontCfg = cfg.getCfgAsset(DS_FRONT_KEY);
        depthStencilState
                .back()
                .failOp(
                        frontCfg.getStaticIntFromClass(
                                VK10.class, DS_FAIL_OP_KEY, DEFAULT_DS_FAIL_OP))
                .passOp(
                        frontCfg.getStaticIntFromClass(
                                VK10.class, DS_PASS_OP_KEY, DEFAULT_DS_PASS_OP))
                .depthFailOp(
                        frontCfg.getStaticIntFromClass(
                                VK10.class, DS_DEPTH_FAIL_OP_KEY, DEFAULT_DS_DEPTH_FAIL_OP))
                .compareOp(
                        frontCfg.getStaticIntFromClass(
                                VK10.class, DS_COMPARE_OP_KEY, DEFAULT_DS_COMPARE_OP));

        ConfigAsset backCfg = cfg.getCfgAsset(DS_BACK_KEY);
        depthStencilState
                .back()
                .failOp(
                        backCfg.getStaticIntFromClass(
                                VK10.class, DS_FAIL_OP_KEY, DEFAULT_DS_FAIL_OP))
                .passOp(
                        backCfg.getStaticIntFromClass(
                                VK10.class, DS_PASS_OP_KEY, DEFAULT_DS_PASS_OP))
    ...
    private void loadStages(ConfigAsset cfg) throws VulkanException {
        ...
        for (int i = 0; i < cfgs.size(); i++) {
            ...
            long shaderModule =
                    ShaderUtil.createShaderModule(
                            device, new File(sCfg.getString(FILE_KEY, DEFAULT_FILE)));

            ShaderUtil.fillShaderStage(
                    stages.get(i),
                    shaderModule,
    ...
    private void loadDynamicStates(ConfigAsset cfg) {
        ...
        for (int i = 0; i < dynamicStatesNames.size(); i++) {
            try {
                dynamicStates.put(
                        i,
                        ResourceUtil.getStaticIntValueFromClass(
